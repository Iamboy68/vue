...
import {articleCommentsService, articleSelectOneSerivce} from '@/api/article.js'
...
onMounted(async () => {
    ...
    let result= await articleSelectOneSerivce(route.params.id)
    article.value=result.data

  let commentslist= await SelectbyIdComments(route.params.id)
    if(commentslist.data!==null){
        showCommentes.value=true
    }
...
import {SelectbyIdComments} from "@/api/comments.js";
...
const getCode= async () => {
    ...
    gettingcode.value = `${countdown}秒后重新获取`;
...
const PostComment=async (from) => {
    if (tokenStore.token !== '') {

        if (commentsform.value.content===''){
            ElMessage("请发表评论")
            return ;
        }

        await articleCommentsService(from)
        ElMessage.success('发表成功')
        commentsform.value.content=''
        router.go(0)
    }else {
        ElMessage("请先登录")
    }
...
<!--                                                            </div>-->
                                                            <div class="follower-item follower-click" @click="router.push('/articleManage')">
                                ...
                                <el-form  :model="form" label-width="auto" class="el-form" v-else>
                                    <div class="input-container">
                                        <div class="input-group">
                                            ...
                                            <el-button class="get-code"  :disabled="isButtonDisabled"  style="border: 0;" @click="getCode()">
                                                {{gettingcode}}
                                            </el-button>
            ...
            <!--主体-->
            <el-main>
                <div class="main-one" style="display: flex;justify-content: center;padding-top: 180px;min-height:500px;">

                    <div style="min-height:300px;width: 80%;margin-top: 10px;background-color: #ffffff">
                        ...
                        <div style="height: 70px ;width: 100%;background-color: #f5f5f5;align-items: center;border-radius: 15px;display: flex;justify-content: space-between">
                            <div style="width: 100%;height: 40px;padding: 10px 0 0 10px">
                                <span>作者：{{ article.nickname }}</span>
                                <span style="margin-left: 200px">于 {{ article.createTime }} 创建</span>
                            </div>
                        </div>
...
<!--                        </div>-->
                ...
                <el-divider style="width: 60%;margin-left:20%"/>

                    <div style="width:60%;min-height: 600px;margin-left: 20%">
                        <el-form :model="commentsform" >
                        <div style="width: 100%;height:110px;display: flex;justify-content: space-between">
                            ...
                            <div style="margin-top: 15px">
                                <el-button style="width: 100px; height: 50px; border-radius: 10px" type="primary" @click="PostComment(commentsform)">
                                    发布
                                </el-button>
                            </div>
                        </div>
                        </el-form>
                        <div v-if="showCommentes">
                            <div style="width: 100%;min-height: 250px;padding: 20px 0 0 20px" >

                                <div v-for="comment in comments" :key="comment.id">
                                    <div style="width: 100%;height: 80px;display: flex;justify-content: left;">
                                        <el-row style="height: 80px;width: 100%;">
                                            <el-col :xs="24" :sm="12" :md="2" >
                                                <div style="align-items: center;width: 70px; height:70px;border-radius: 50%;" >
                                                    <el-image style="width: 100%; height: 100%;border-radius: 50%" :src="avatar" fit=scale-down />
                                                </div>
                                            </el-col>
                                            <el-col :xs="24" :sm="12" :md="7" >
                                                <div style="margin: 0;align-items: center;width: 100%; height:100%;" >
                                                    <div style="height: 60%;width: 100%;">
                                                        <h3 style="margin: 0;padding: 5px 0 0 15px">
                                                            {{ comment.nickname }}
                                                        </h3>
                                                    </div>
                                                    <div style="height: 40%;width: 100%;">
                                                        <p style="margin: 0;padding: 1px 0 0 15px">
                                                            {{ comment.createTime }}
                                                        </p>
                                                    </div>
                                                </div>
                                    ...
                                    <div style="width: 70%;min-height: 150px;margin:20px 0 0 15%;">
                                        <p style="margin: 0">
                                            {{comment.content }}
                                        </p>
                                    </div>
